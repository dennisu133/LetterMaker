		...
		<ComboboxPrimitive.Portal>
			...
			>
				<ComboboxPrimitive.Popup
					...
					data-chips={!!anchor}
					className={cn(
						"bg-popover text-popover-foreground data-open:animate-in data-closed:animate-out data-closed:fade-out-0 data-open:fade-in-0 data-closed:zoom-out-95 data-open:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 ring-foreground/10 *:data-[slot=input-group]:bg-input/30 *:data-[slot=input-group]:border-input/30 group/combobox-content relative max-h-72 w-(--anchor-width) max-w-(--available-width) min-w-36 origin-(--transform-origin) overflow-hidden rounded-none shadow-md ring-1 duration-100 data-empty:shadow-none data-empty:ring-0 data-[chips=true]:min-w-(--anchor-width) *:data-[slot=input-group]:m-1 *:data-[slot=input-group]:mb-0 *:data-[slot=input-group]:h-8 *:data-[slot=input-group]:shadow-none",
						className
					)}
					{...props}
				/>
...
// ============================================================================
// Free-form Combobox (autocomplete-style input that accepts custom values)
// ============================================================================

interface ComboboxFreeFormContextValue {
	inputValue: string;
	setInputValue: (value: string) => void;
}

const ComboboxFreeFormContext = React.createContext<ComboboxFreeFormContextValue | null>(null);

function useComboboxFreeForm() {
	const context = React.useContext(ComboboxFreeFormContext);
	if (!context) {
		throw new Error("useComboboxFreeForm must be used within ComboboxFreeForm");
	}
	return context;
}

interface ComboboxFreeFormProps extends Omit<
...
/**
 * A combobox variant that allows free-form text input with autocomplete suggestions.
 * Unlike the regular Combobox, this accepts any typed value, not just items from the list.
 * Use this for fields like salutation/closing where users can pick from suggestions OR type custom text.
 */
function ComboboxFreeForm({
	...
	return (
		<ComboboxFreeFormContext.Provider value={{ inputValue, setInputValue }}>
		...
		</ComboboxFreeFormContext.Provider>
	);
}

/**
 * Input component for ComboboxFreeForm that maintains the typed value.
 */
function ComboboxFreeFormInput({
	className,
	children,
	disabled = false,
	showTrigger = true,
	...
	triggerAriaLabel,
	...props
}: Omit<ComboboxPrimitive.Input.Props, "value" | "onChange"> & {
	showTrigger?: boolean;
	showClear?: boolean;
	triggerAriaLabel?: string;
}) {
	const { inputValue, setInputValue } = useComboboxFreeForm();
